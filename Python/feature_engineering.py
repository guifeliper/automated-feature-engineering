...
def crisp(rawCSVdata, fileName):
#PREPARE THE DATA
    numerics = ['int16', 'int32', 'int64', 'float16', 'float32', 'float64']
    df = rawCSVdata.select_dtypes(include=numerics).drop('class', axis=1, errors='ignore')


# FEATURE GENERATION
    operators = ["sum", "diff", "mult", "ratio"]
    newFeatures = featureGeneration(df, operators)
    df_new_features = pd.concat([df, newFeatures, rawCSVdata['class']], axis=1, sort=False)
    df_base_numeric = pd.concat([df, rawCSVdata['class']], axis=1, sort=False)
...
def main():
    ...
    for i, pathFile in pathFiles.iterrows():
        ...
        metaData.loc[pathFile[0]] = crisp(rawCSVdata, pathFile[0])
